/*
 * Fireplace status screen, peripheral half — portrait 128x32 OLED:
 * battery + connection state on top, the same campfire below.
 *
 * Event-listener plumbing adapted from the stock ZMK nice_view widgets
 * (Copyright (c) 2023 The ZMK Contributors, MIT).
 *
...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    lv_canvas_draw_text(canvas, 0, 16, CANVAS_SIZE, &label_dsc,
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
    ...
    /* the canvases don't cover the whole area — keep the gap dark */
    ...
    /* child 0: battery/connection square, viewer top = framebuffer right */
